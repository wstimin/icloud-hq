...
- 管理员 TOTP 双重认证
- 子邮箱可绑定第三方平台 TOTP，前台显示当前 6 位动态码和刷新倒计时
- 前台持有查询密钥者可绑定或更换子邮箱 2FA，后台可绑定、更换和删除
...
这里的子邮箱 2FA 是第三方平台在注册或开启双重认证时提供的 TOTP 配置，不是 Apple 受信任设备或手机号收到的验证码，也不是管理员登录后台使用的 TOTP。

1. 在第三方平台开启身份验证器 2FA，找到 Base32“手动设置密钥”，或复制完整的 `otpauth://totp/...` 地址。
2. 可在管理后台“子邮箱”列表点击“绑定 2FA”，也可先在公开首页输入该子邮箱的查询密钥，再点击“绑定 2FA”。
3. 粘贴手动密钥或 `otpauth://` 地址并保存。
4. 公开首页将“邮箱验证码”和“2FA 验证码”分为两个独立页签；进入“2FA 验证码”页签，用同一查询密钥查看当前 6 位 TOTP 动态码。
5. 动态码每 30 秒自动更新；绑定新配置会覆盖该子邮箱当前的 2FA 配置。

邮件验证码查询接口不会返回 2FA，2FA 查询接口也不会返回邮件内容。前台和管理后台操作的是数据库中的同一份加密 TOTP 配置：前台绑定或更换后，管理后台会自动同步状态；后台更换或删除后，前台下一次查询或动态码刷新时同步。

首版只支持最常见的标准 TOTP：`SHA1`、`6` 位、`30` 秒周期，不支持 HOTP。若平台只显示二维码，请在平台界面中寻找“无法扫描”“手动输入”或“设置密钥”；本系统不会从邮件中自动提取 2FA 二维码。

TOTP 原始密钥和查询密钥的可恢复副本使用 `MASTER_KEY_HEX` 进行 AES-256-GCM 加密。敏感信息不会进入后台常规状态接口；管理员必须重新输入当前登录密码，才能按需查看查询密钥、TOTP 手动密钥和当前动态码。任何持有该子邮箱查询密钥的人都可以查看动态码并更换其 2FA，因此查询密钥应只交给可信使用者；需要收回权限时请在后台重置查询密钥。
...
- App 专用密码、邮件验证码和子邮箱 TOTP 密钥使用 AES-256-GCM 加密。
...
主加密密钥 `MASTER_KEY_HEX` 丢失后，已保存的 App 专用密码、验证码和 TOTP 密钥无法解密。`TOKEN_PEPPER_HEX` 改变后，所有现有查询密钥都会失效。请加密备份 `.env`，不要提交到 Git。
